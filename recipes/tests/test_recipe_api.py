...
class RecipeAPIv2TestMixin(RecipeMixin):
    ...
    def test_recipe_api_list_returns_status_code_200(self):
        self.assertEqual(
            self.get_recipe_api_list().status_code,
            200
        )
    ...
    def get_auth_data(self, username="user", password="password"):
        ...
        user = self.make_author(username=userdata.get(
            'username'), password=userdata.get('password'))
        response = self.client.post(
            reverse('recipes:token_obtain_pair'),
            data=userdata)
        return {
            "jwt_access_token": response.data.get('access'),
            "jwt_reflesh_token": response.data.get('reflesh'),
            "user": user,
        }
    ...
    def get_recipe_raw_data(self):
        data = {
            ...
            "preparation_steps": "testando o funcionanmendo da API REST"
        }
        return data
...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    def test_recipe_api_list_loads_correct_of_recipes(self):
    ...
    def test_recipes_api_list_do_not_show_not_plubished_recipes(self):
    ...
    def test_recipes_api_list_can_loads_recipes_by_category_if_category_id(self):
        # Creates Categorys
    ...
    def test_jwt_login(self):
        access_token = self.get_auth_data()["jwt_access_token"]

        response = self.client.post(
            reverse("recipes:token_verify"),
            data={
                "token": access_token
            }
        )
    ...
    def test_recipe_api_list_logged_user_can_update_a_recipe_owned_by_another_user(self):
        ...
        # This user cannot update the recipe because is owned by another user
        # user
        another_user = self.get_auth_data(username="cant_update")
        jwt_access_token_from_anoher_user = another_user.get(
            "jwt_access_token")
        ...
        response = self.client.patch(
            ...
            HTTP_AUTHORIZATION=f'Bearer {jwt_access_token_from_anoher_user}'
        )
